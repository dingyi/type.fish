...
import { type TypefaceClassification, useTypefaces } from "@/data/typefaces";
...
export function TypefaceGrid() {
  const { data: typefaces, isLoading, isError, error } = useTypefaces();
  ...
  // license=open-source in the URL is the Open Source preset. Reading it here
  // keeps the URL the single source of truth for that filter.
  const openSourceOnly = searchParams.get("license") === "open-source";
...
function SkeletonControls() {
  return (
    <div className="mb-6 space-y-3">
      <div className="h-7 w-64 animate-pulse rounded-full bg-muted/60" />
      <div className="h-9 w-full animate-pulse rounded-full bg-muted/40" />
    </div>
  );
}
